//! Стратегии фрагментации TLS ClientHello. Чистая логика без сети — переносима
//! на любую платформу.
...
/// Порядок перебора при автоподборе: от безобидного к агрессивному.
...
/// Записать `buf` в апстрим, применив стратегию фрагментации.
