//! Минимальный SOCKS5-сервер (только CONNECT). Первый «полёт» апстрима
//! (TLS ClientHello) прогоняется через [`tamper::apply`], дальше — прозрачный
//! двунаправленный копир.
...
use crate::tamper::{Strategy, apply};
