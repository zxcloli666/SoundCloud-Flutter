//! Автоподбор стратегии: параллельно поднимаем по SOCKS на каждую стратегию из
//! [`LADDER`], дёргаем `probe_url` и берём первую, что ответила успехом.
...
use crate::tamper::{LADDER, Strategy};
            ...
            tokio::select! {
                ...
                r = client.head(&url).send() => match r {
                    Ok(resp) if resp.status().is_success() || resp.status().is_redirection() => {
                        info!(?s, status = %resp.status(), "dpi-bypass: strategy works");
                        let _ = tx.send(s).await;
                    }
                    Ok(resp) => debug!(?s, status = %resp.status(), "dpi-bypass: bad status"),
                    Err(e) => debug!(?s, ?e, "dpi-bypass: probe err"),
                }
            }
        });
